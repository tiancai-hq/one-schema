...
const validateByType = {
  bool: (input, schema) => {
    let validated = typeof input === "boolean";
    let error = null;
    if (!validated) {
  ...
  int32: (input, schema) => {
    ...
    if (!schema.settings.allowNaN && input !== ~~input) {
      return {
        validated: false,
  ...
  float64: (input, schema) => {
    let validated = typeof input === "number" && !Number.isInteger(input);
    let error = null;
    if (!validated) {
      return {
        validated,
  ...
  string: (input, schema) => {
    let validated = typeof input === "string";
    let error = null;
    if (!validated) {
  ...
  object: (input, schema, options) => {
    let validated = typeof input === "object";
    let error = "";
    if (!validated) {
    ...
    let result = sanitizeKeys(input, schema.settings.schema);
    if (!result.validated) {
